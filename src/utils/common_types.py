...
class BERT_para(object):
    ...
    def get_eval_data_file(self, task):
        return config.RESULT_PATH / f"{self.output_folder}/eval_data_{task}_{self.mode}_{self.prob_thresholds}_top{self.top_n}.log"

    def get_eval_item_file(self, task):
        return config.RESULT_PATH / f"{self.output_folder}/eval_items_{task}_{self.mode}_{self.prob_thresholds}_top{self.top_n}.log"

    def get_pred_data_file(self, task):
        return config.RESULT_PATH / f"{self.output_folder}/pred_data_{task}_{self.mode}_{self.prob_thresholds}_top{self.top_n}.log"

    def get_pred_item_file(self, task):
        return config.RESULT_PATH / f"{self.output_folder}/pred_items_{task}_{self.mode}_{self.prob_thresholds}_top{self.top_n}.log"