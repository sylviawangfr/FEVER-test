...
def train_dbpedia():
    ...
    parallel = True
    # Create training and test sets.
    data_train, data_dev = read_data_in_file_row()
    ...
    train_data_loader = DataLoader(data_train, batch_size=32, shuffle=True, collate_fn=collate_convert_to_dgl)
...
def read_data_in_file_row():
    data_train = read_all_files(config.RESULT_PATH / "sample_ss_graph_train")
    data_dev = read_all_files(config.RESULT_PATH / "sample_ss_graph_train")
    # print(f"train data len: {len(data_train)}; eval data len: {len(data_dev)}\n")
    return data_train, data_dev
