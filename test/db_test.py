...
class TestDB(unittest.TestCase):
    def test_multithread_query(self):
        ...
        list_d = read_json_rows(config.DATA_ROOT / "wiki-pages/wiki-001.jsonl")[1:1000]
        list_id = [item['id'] for item in list_d]
        qr = []
        thread_exe(lambda x:qr.append(x) if get_all_sent_by_doc_id_mutithread(one_conn, x) == ([], []) else None, list_id, 20, "testing multithread query")
