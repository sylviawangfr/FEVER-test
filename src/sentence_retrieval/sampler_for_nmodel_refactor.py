...
def sample_single_item(item, additional_data_dict, pred=False):
    item_data_list = []
    ...
    if not pred:
        ...
        if all_evidence_set is not None:
            for doc_id, ln in all_evidence_set:
                _, text, _ = fever_db.get_evidence_multithread(one_conn, doc_id, ln)
                r_list.append(text)
    ...
    for doc_id in doc_ids:
        cur_r_list, cur_id_list = fever_db.get_all_sent_by_doc_id_mutithread(one_conn, doc_id, with_h_links=False)
    ...
    for i, sent_item in enumerate(all_sent_list):
        ...
        item_data_list.append(sent_item)
...
def get_full_list(tokenized_data_file, additional_data_file, pred=False, top_k=None):
    ...
    :return:
    """
    d_list = read_json_rows(tokenized_data_file)[0:10]
