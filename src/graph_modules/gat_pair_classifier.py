...
class GATClassifier(nn.Module):
    ...
    def forward(self, bgp):
        # For undirected graphs, in_degree is the same as
        # out_degree.
        bg1, bg2 = bgp[0], bgp[1]
        h1 = bg1.in_degrees().view(-1, 1).float()
        h2 = bg2.in_degrees().view(-1, 1).float()
        ...
        bg1.ndata['h'] = h1
        bg2.ndata['h'] = h2
        hg1 = dgl.mean_nodes(bg1, 'h')
        hg2 = dgl.mean_nodes(bg2, 'h')
...
def collate(samples):
    ...
    batched_graph1 = dgl.batch(g1_l)
    batched_graph2 = dgl.batch(g2_l)
