...
def tfidf_to_graph_sampler(tfidf_data):
    ...
    with tqdm(total=sample_dataloader.max_bunch_number, desc=f"Sampling") as pbar:
        for batched_sample in sample_dataloader:
            ...
            pbar.update(1)
            batch += 1
            objgraph.show_refs([sample_tfidf], filename=config.LOG_PATH / "sample-graph.png")
            objgraph.show_backrefs([sample_tfidf], filename=config.LOG_PATH / 'sample-graph.png')
            del sample_tfidf
            del paras
            del batched_sample
            rt = gc.collect()
...
def test_memory():
    ...
    for _, i in enumerate(tt):
        print(f"i: {len(i)}")
        paras.upstream_data = list(range(500*500))
        del i
        # del para
        paras.upstream_data = []
        gc.collect()
