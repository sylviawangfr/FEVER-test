...
def wiki_finetune():
    ...
    tokenizer = BertTokenizer.from_pretrained('bert-large-uncased', do_lower_case=do_lower_case)
    ...
    # Prepare model
    model = BertForPreTraining.from_pretrained(pretrained_model_name_or_path)
    if fp16:
        model.half()
    model.to(device)
    if local_rank != -1:
