...
class GATClassifier2(nn.Module):
    def __init__(self, in_dim, hidden_dim, num_heads, n_classes):
        super(GATClassifier2, self).__init__()
    ...
    def forward(self, graph1_batched, graph2_batched):
        ...
        h_n1 = bg1.ndata['nbd_align']    # aligned node embeddings: 768
        h_e1 = bg1.edata['ebd']  # 768
        h_n2 = bg2.ndata['nbd_align']    # aligned node embeddings: 768
        h_e2 = bg2.edata['ebd']  # 768
        ...
        bg1.ndata['h'] = h_n1
        bg2.ndata['h'] = h_n2
        hg_all1 = dgl.mean_nodes(bg1, 'h')
        hg_all2 = dgl.mean_nodes(bg2, 'h')
        x = torch.cat([hg_all1, hg_all2], dim=1)
